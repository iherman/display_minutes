# Display minutes' information and, optionally, real person names instead of nicknames

At the moment, the minutes generated for W3C calls (using the "standard" `scribe` tool) are spread over W3C's date space. Unfortunately, there is no single place where all the minutes could be seen altogether, to see their original agenda, or to see the resolutions that have been passed. Also, only W3C staff can modify those scripts (it has become extremely rare to grant commit right to the CVS repository for non-staff persons) This small utility is built on top of the standard W3C minutes to make this possible. It relies on the assumption that the list of all minutes can be located in the same (GitHub) repository where this tool resides; it generates two files:
...
> The current repository keeps the (relevant) structure of an example deployment; all files in the `minutes` directory are meant as an example only. 
...
- `nicknames` (optional): reference to a JSON object mapping IRC nicknames to real person's name (see below).
...
The script is in TypeScript, and has been developed and deployed using [deno](https://deno.land). It consists of five files in the `./lib` directory, plus three "runners" in the top directory:

- `lib/gen_index.ts`: module to deploy the information on minutes by filling in the dedicated "slots" in two template files: `templates/index_template.html` and `templates/resolutions_template.html`. The results are stored in the `index.html` and `resolutions.html` files, respectively. The `params.json` parameter file (see above) can be used to find the minutes within the repository.
- `lib/nicknames`: module to collect all the IRC nicknames used in a meeting, and exchange them to real names (provided the nickname mapping file is available, see above)
...
- `run_index.ts`: run the `lib/gen_index.ts` module
- `run_nicknames.ts`: run the `lib/nicknames.ts` module
- `run_all.ts`: run both the `lib/gen_index.ts` and `lib/nicknames.ts` sequentially

(The reason to have separate "runners" is to give some flexibility in using these script remotely from a GitHub action. See the `deno.jsonc` file for details.)
  ...
  deploy:
    ...
    steps:
      ...
      - name: Generate the index files
        run: |
          (cd script; deno run -A run_index.ts)
...
This approach pre-supposes that this `script` directory has been copied into the GitHub repository of the Working Group. This is suboptimal in case new version of this library is published. An alternative is to run the script directly from `jsr` without a local installation. This alternative script is as follows:
    ...
    steps:
      ...
      - name: Setup Deno
        ...
        with:
          deno-version: v2.x
      - name: Generate the index files
        # Details depend on where the script is located; in this case, the 
        # directory only contains the template files, the parameters, possibly
        # the nickname mapping file
        run: |
          # Run the index generation
          (cd script; deno run -A jsr:@iherman/display-minutes)
          # If the nickname replacement are also needed:
          (cd script; deno run -A jsr:@iherman/display-minutes/nicknames)
          mv script/index.html minutes/index.html
          mv script/resolutions.html minutes/resolutions.html
      - name: Setup Github Pages
        uses: actions/configure-pages@v5
...
The script above does the index generation _and_ the nickname replacements in two separate steps. Alternatively, using `jsr:@iherman/display-minutes/full` can also be used to execute both.
